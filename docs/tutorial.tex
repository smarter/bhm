...
\subsection{TKFAC (Trace-normalised K-FAC)}

TKFAC scales each K-FAC factor by the trace of the other:
\[
  \mathbf{A}_\ell^{T} = \mathbf{A}_\ell \cdot \tr(\mathbf{S}_\ell),
  \qquad
  \mathbf{S}_\ell^{T} = \mathbf{S}_\ell \cdot \tr(\mathbf{A}_\ell).
\]
Since scalar multiplication preserves eigenvectors, the TKFAC
eigenbasis is identical to K-FAC's.  The net effect is a uniform
scaling of all K-FAC eigenvalues:
\[
  \lambda_{jm}^{T}
  = \tr(\mathbf{A}_\ell)\;\tr(\mathbf{S}_\ell)\;
    \lambda_{\mathbf{S},j}\;\lambda_{\mathbf{A},m}.
\]
This can better match the overall scale of the true curvature block
without changing its directional structure.

\subsection{ETKFAC (Eigenvalue-corrected TKFAC)}

ETKFAC uses TKFAC's trace-scaled factors for the eigendecomposition,
then applies EK-FAC-style eigenvalue corrections in that basis.

\medskip
\noindent\textbf{Equivalence to EK-FAC.}\quad
Since trace scaling preserves eigenvectors, the ETKFAC eigenbasis is
identical to both TKFAC's and K-FAC's.  The eigenvalue corrections
are computed from per-sample projections into the eigenbasis and do
not depend on the factor scaling.  Therefore ETKFAC produces the
same matrix as EK-FAC (up to numerical precision).
